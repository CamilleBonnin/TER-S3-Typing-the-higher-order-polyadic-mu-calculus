...
(* Un element d'un environnement de typage incomplet, sans les variances *)
type incomplete_type_assignement = {
  phi : formula;
  tau : mu_type
}

(* Un environnement de typage incomplet, sans les variances *)
type incomplete_typing_environment =
  incomplete_type_assignement list
...
(* Transforme element d'un environnement de typage incomplet en chaine de caracteres *)
let ita_to_string (ita : incomplete_type_assignement) : string =
  f_to_string ita.phi ^ " : " ^ t_to_string ta.tau

(* Transforme un environnement de typage incomplet en chaine de caracteres *)
let rec ite_to_string (delta : typing_environment) : string =
  match delta with
    | [] -> ""
    | ita::g -> ita_to_string ita ^ "\n" ^ te_to_string g
