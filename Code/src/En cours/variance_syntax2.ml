...
let variance_from_string (s : string) : variance = 
  match s with 
    ...
    | s when (String.equal (String.lowercase_ascii s) "novariance") -> Novariance
...
let all_variances : variance list = 
  [None ;NAdditive; Additive; NJoin;Any ;NMeet; Join; Meet; Monotone; Antitone ; Novariance]